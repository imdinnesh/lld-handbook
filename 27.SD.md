...
sequenceDiagram
    ...
    Customer->>OrderController: placeOrder(items)
    OrderController->>OrderService: createOrder(items)
    OrderService->>Database: save(order)
    Database-->>OrderService: orderId
    OrderService-->>OrderController: orderConfirmation
    OrderController-->>Customer: confirmation page
```
...
sequenceDiagram
    ...
    deactivate AuthService
    AuthController-->>User: loginResponse
...
sequenceDiagram
    ...
    Note over OrderService: OrderService continues<br/>without waiting
    MessageQueue-)NotificationService: deliver(orderPlaced event)
    NotificationService->>NotificationService: sendEmail(customer)
```
...
sequenceDiagram
    ...
    Note over AuthService: Check expiry, signature,<br/>and permissions
    AuthService-->>AuthService: isValid
```
...
sequenceDiagram
    ...
    AuthService->>Session: new Session(userId, expiry)
    activate Session
...
sequenceDiagram
    ...
    activate AuthService
    AuthService->>Session: invalidate()
    activate Session
    Session-->>AuthService: confirmed
    deactivate Session
    Note over Session: Session destroyed
    destroy Session
    AuthService-->>User: logoutSuccess
    deactivate AuthService
```
...
sequenceDiagram
    ...
    alt credentials valid
        ...
        AuthController-->>User: loginSuccess(token)
    else credentials invalid
        AuthService-->>AuthController: authError
        AuthController-->>User: loginFailed(error)
    end
...
sequenceDiagram
    ...
    loop for each item in cart
        CartService->>InventoryService: checkStock(itemId, quantity)
        activate InventoryService
        InventoryService->>Database: getAvailableStock(itemId)
        activate Database
        Database-->>InventoryService: stockCount
        deactivate Database
        InventoryService-->>CartService: stockStatus
...
sequenceDiagram
    ...
    Customer->>OrderController: checkout(order, promoCode)
    activate OrderController

    opt promoCode provided
        OrderController->>DiscountService: validatePromo(promoCode)
        activate DiscountService
        DiscountService-->>OrderController: discountAmount
        deactivate DiscountService
    end

    OrderController->>OrderService: finalizeOrder(order, discount)
    activate OrderService
    OrderService-->>OrderController: orderConfirmation
    deactivate OrderService

    OrderController-->>Customer: confirmationPage
...
sequenceDiagram
    ...
    and track analytics
        OrderService->>AnalyticsService: trackOrderPlaced(order)
