...
The `<<abstract>>` stereotype tells you this class can't be instantiated directly. Looking at the methods, `getArea()` and `getPerimeter()` are abstract (shown in italics/with asterisks), meaning `Shape` declares them but provides no implementation.
...
```mermaid
graph LR
    Dependency[Dependency] --> Association[Association] --> Aggregation[Aggregation] --> Composition[Composition] --> Inheritance[Inheritance] --> Realization[Realization]
    
    style Dependency fill:#f9f,stroke:#333,stroke-width:1px
    style Realization fill:#bbf,stroke:#333,stroke-width:1px
```
