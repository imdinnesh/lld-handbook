...
> [!TIP]
> In LLD interviews, you don't always need to draw a formal use case diagram. But spending some time listing actors and use cases before diving into class design shows structured thinking and saves you time during the design and implementation phase.
...
1. **Start with a verb**: "Book Ticket" not "Ticket Booking." The verb makes the action clear.
2. **Represent a complete goal**: "Book Ticket" is a complete goal. "Enter Credit Card Number" is a single step within a goal, too granular for a use case diagram.
3. **Deliver a meaningful result**: After "Book Ticket" completes, the actor has a confirmed booking. That's a meaningful outcome.
...
graph LR
    ...
    subgraph MovieSystem["Movie Ticket Booking System"]
...
```mermaid
graph LR
    Customer["👤 Customer"] --- BookTicket(["Book Ticket"])
```
...
```mermaid
graph LR
    Checkout(["Checkout"]) -.->|&lt;&lt;include&gt;&gt;| ValidatePayment(["Validate Payment"])
```
...
"Apply Coupon" extends "Book Ticket." A customer can book a ticket without applying a coupon. But if they have a coupon, they can optionally apply it during booking. The dashed arrow with the label `<<extend>>` points from the extending use case back to the base use case.

```mermaid
graph LR
    ApplyCoupon(["Apply Coupon"]) -.->|&lt;&lt;extend&gt;&gt;| BookTicket(["Book Ticket"])
```
...
graph BT
    ...
    PayByCard(["Pay by Card"]) --> MakePayment(["Make Payment"])
    PayByUPI(["Pay by UPI"]) --> MakePayment(["Make Payment"])
```
...
* **System-level / Secondary**: Process Payment (called during booking)

### Step 3: Define the System Boundary
Draw a box around all the use cases and label it with the name of your system. This box represents your system and visually defines the scope. Everything inside the box is your responsibility to implement; anything outside is external.
...
graph LR
    ...
    subgraph MovieTicketBooking["Movie Ticket Booking System"]
    ...
    %% Admin Connections
    Admin --- AddMovie
    Admin --- UpdateShowtime
    
    %% Relationships between Use Cases
    BookTicket -.->|&lt;&lt;include&gt;&gt;| MakePayment
    FilterGenre -.->|&lt;&lt;extend&gt;&gt;| BrowseMovies
