...
2. What events cause transitions between states?
3. What actions happen during those transitions?

```mermaid
stateDiagram-v2
    [*] --> StateA : action1()
    StateA --> StateB : action2()
    StateB --> StateC : action3()
    StateA --> [*] : cancel()
    StateB --> [*] : cancel()
    StateC --> [*] : cancel()
```
...
### 1.1 Model object lifecycles clearly
...
### 1.2 Catch missing transitions and invalid states
...
### 1.3 Bridge between requirements and implementation
...
```mermaid
stateDiagram-v2
    HasMoney --> Dispensing : selectProduct [itemInStock]
    HasMoney --> Refunding : selectProduct [itemOutOfStock]
```
...
```mermaid
stateDiagram-v2
    [*] --> HasMoney : insertMoney
    HasMoney --> HasMoney : insertMoreMoney / updateBalance
```
...
stateDiagram-v2
    ...
    Validating --> validation_choice : validationComplete
    validation_choice --> Approved : [score > 80]
    validation_choice --> Rejected : [score < 30]
    validation_choice --> ManualReview : [30 <= score <= 80]
```
